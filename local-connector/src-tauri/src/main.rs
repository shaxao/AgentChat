...
use ide::{
    handle_deep_link as handle_ide_deep_link,
...
fn connector_diagnostics(state: State<'_, SharedState>) -> String {
    ...
    let connected_count = guard.runners.values().filter(|h| h.status.connected).count();
    format!(
    ...
    if let Some(session_id) = &state.last_active_session {
        if let Some(handle) = state.runners.get(session_id) {
            ...
            if !active_grant_id.is_empty() {
                if let Some(grant) = grants.iter().find(|grant| grant.grant_id == active_grant_id) {
                    return Some(grant.clone());
                }
            }
            let active_project = handle.config.project.trim();
            if !active_project.is_empty() {
                if let Some(grant) = grants.iter().find(|grant| grant.project_root == active_project) {
                    return Some(grant.clone());
                }
            }
        }
...
#[tauri::command]
fn update_local_project_grant_server(grant_id: String, server_base: String) -> Result<connector::LocalProjectGrant, String> {
    connector::update_local_project_grant_server(&grant_id, &server_base)
}

#[tauri::command]
fn disconnect_session(state: State<'_, SharedState>, session_id: String) -> Result<UiStatus, String> {
...
#[tauri::command]
fn delete_local_project_grant(state: State<'_, SharedState>, grant_id: String) -> Result<UiStatus, String> {
    ...
    let pending_ids = guard
        .pending_launches
        .iter()
        .filter(|(_, config)| config.grant_id == removed_grant_id || config.project == removed_project)
        .map(|(sid, _)| sid.clone())
        .collect::<Vec<_>>();
...
mod tests {
    ...
    fn status(session: &str, project: &str, connected: bool, needs_project: bool) -> UiStatus {
        UiStatus {
            status: if needs_project { "project_required" } else { "connected" }.to_string(),
            detail: String::new(),
